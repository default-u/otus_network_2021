...
**Схема L2 пиринга:**  
![image](https://user-images.githubusercontent.com/8961955/150991627-f953fbb1-5281-49d0-9901-44148d08ea11.png)
...
Итого, поставленная задача "мигрировать телефонные кластера в Мск. и разместить в двух ЦОД без смены адресации" выполнена, в существующую схему маршрутизации добавлен новый функционал, хост 192.168.1.34 успешно перенесен из AS65013 в AS65011, в AS65012 добавлен хост 192.168.1.13, связь между всеми устройствами в сети 192.168.1.0/24 работает. Резервирование каналов связи осуществляется на уровне L3 BGP. Базовая связность L3 между сетями 172.16.N.N также функционирует.    
...
**update:**  
Вопрос связи между 192.168.1.0/24 на площадках AS65012 и AS65013 решен упрощением конфигурации - исключен vrf VXLAN.  
Конфигурация leaf коммутаторов в AS65012 и AS65013 приведена к виду:  

<details> 
<summary>Изменный фрагмент конфигурации  </summary>
<pre><code>
interface Vlan20  
  no shutdown  
  ip address 192.168.1.N/24  
